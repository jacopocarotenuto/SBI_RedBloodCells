...
from numpy import arange, float32, int64, sqrt, zeros
...
def Simulator_noGPU(dt, DeltaT, TotalT, theta, transient_time=0, i_state=None):
    ...
    sqrt_2_mu_x_D_x_dt = sqrt(2 * mu_x * D_x * dt)
    sqrt_2_mu_y_D_y_dt = sqrt(2 * mu_y * D_y * dt)
    sqrt_2_eps_2_dt_tau = sqrt(2 * eps**2 * dt / tau)
    mu_x_dt = mu_x * dt
    mu_y_dt = mu_y * dt
    dt_on_tau = dt / tau
    ...
    for t in arange(time_steps_amount - 1):
        x[:,] = (
            x[:,]
            + mu_x_dt * (-k_x * x[:,] + k_int * y[:,])
            + sqrt_2_mu_x_D_x_dt * np.random.randn(n_sim, 1)
        )
        y[:,] = (
            y[:,]
            + mu_y_dt * (-k_y * y[:,] + k_int * x[:,] + f[:,])
            + sqrt_2_mu_y_D_y_dt * np.random.randn(n_sim, 1)
        )
        f[:,] = (
            f[:,]
            + (-f[:,])* dt_on_tau
            + sqrt_2_eps_2_dt_tau * np.random.randn(n_sim, 1)
        )
...
def CheckParameters(dt, DeltaT, TotalT, theta):
    ...
    print(
        f"Your Sampling Time (Delta) is {DeltaT:.2E} seconds, corresponding to a {1/DeltaT:.2f}Hz sampling frequency"
    )
    ...
    print(
        f"The gap between two sampled points is {sampling_delta_time_steps:.1E} time steps"
    )
...
class SimulationPipeline:
    def __init__(
        ...
        if os.getcwd()[-12:] != "LCPb-Project":
            print("WARNING: You are not in the LCPb-Project folder. Change directory to the project root folder.")
            os.chdir("..")
            assert os.getcwd()[-12:] == "LCPb-Project", "Error when changing directory to the project root folder.\n Please execute the script from the project root folder."

        if not os.path.exists("Data"):
            print("WARNING: Data folder not found. Creating Data folder")
            os.makedirs("Data")
...
class SimulationLoader:
    def __init__(self, day_folder=None) -> None:
        ...
        self.simulation_files = [
            os.path.join(self.today_folder_path, i)
            for i in os.listdir(self.today_folder_path)
        ]
